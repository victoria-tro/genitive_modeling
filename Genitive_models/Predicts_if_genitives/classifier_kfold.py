...
# Import dataset
df = pd.read_csv('dataset/bm_non_genitive_genitive_balanced_4000.csv', header=None, sep=',',names=['bm', 'class'],encoding='UTF-8',skipinitialspace=True,on_bad_lines='skip')   # columns names if no header
...
# Class values: 0 and 1.
# 0 = This sentence does not contain a genitive construction, e.g. Dette er en setning
# 1 = This sentence contains a genitive construction, e.g. Dette er byråets idé
y_data = df.iloc[:, -1] 
...
my_sent = ["I forbindelse med Stortingets behandling av St.meld"]
