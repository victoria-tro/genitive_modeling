...
# Import dataset
df = pd.read_csv('dataset/tagged_genitives.csv', header=None, sep='\t',names=['bm', 'class'],encoding='UTF-8',skipinitialspace=True,on_bad_lines='skip')   # columns names if no header
...
# Class values: 1 and 2.
# 1 = Keep the genitive construction as is, e.g. dagens fangst -> dagens fangst
# 2 = Rewrite the genitive construction, e.g. byråets idé -> idéen til byrået
y_data = df.iloc[:, -1] 


print("TRANSFORMING TEXT DATA ...")
vect = CountVectorizer(analyzer = 'char',ngram_range=(4,4)) # This is a bag-of-words technique
x_data = vect.fit_transform(x_data)

print('\nTRAINING ...\n')

# We use k-fold cross validation to look for overfitting
# We use the model Logistic Regression because it performs well on this type of data
scores=[]
...
# Save model
print("SAVING MODEL ...")
model_pkl_file = "models/rewrite_or_not_model.pkl"  
...
with open('models/rewrite_or_not_model.pkl', 'rb') as file:
...
# Test model on single sentence
# This is also how you can use the loaded model
my_sent = ["I forbindelse med dagens fangst"]
